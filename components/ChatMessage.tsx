...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, sources, isLatest, onTypingUpdate }) => {
  ...
  const cleanText = (text: string) => {
    // Aggressively remove all asterisks and markdown noise
  ...
  return (
    <div className={`flex w-full mb-8 ${isUser ? 'justify-end' : 'justify-start'} animate-modern-fade`}>
      <div className={`flex max-w-[95%] md:max-w-[85%] ${isUser ? 'flex-row-reverse' : 'flex-row'} gap-3 md:gap-5 group`}>
        {/* Avatar */}
        <div className={`w-10 h-10 md:w-12 md:h-12 rounded-2xl flex-shrink-0 flex items-center justify-center text-sm shadow-lg transition-all duration-300 group-hover:scale-110 ${
          isUser ? 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rotate-1' : 'bg-emerald-600 text-white -rotate-1'
        }`}>
          {isUser ? (
        ...
        <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
          <div className={`px-5 md:px-7 py-4 md:py-5 rounded-[1.5rem] md:rounded-[2rem] text-[15px] md:text-[16px] leading-[1.6] shadow-md relative transition-all ${
            isUser 
